...
def inference_from_folder(img_name: str = Form(...), conf: str = Form(...)):
    conf = float(conf)
    # Perform inference
    img_path = f"./Streamlit_Image_Folder/{img_name}"

    # img np array
    img, temp = perform_inference(img_path, conf)
...
@app.post('/inference_input')
def inference_from_folder(img: bytes = File(...), conf: str = Form(...)):
    conf = float(conf)
    # Perform inference
    img_array = np.array(Image.open(io.BytesIO(img)))

    # img np array
    img, temp = perform_inference_input(img_array, conf)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
...
def perform_inference_input(uploaded_file, confidence):
    # Load YOLO-NAS Model
    model = models.get(
        'yolo_nas_m',
        num_classes=28,
